...
\paragraph{Optimal construction~\cite{pagh2005}.}
Pagh, Pagh, and Rao showed that the space lower bound of $-\log_2 \fprate$
bits per element is achievable with polynomial construction
time~\cite{carter1978}, using a two-level hashing scheme.
The BHF achieves the same bound via a simpler (but exponential-time)
exhaustive search, while the adaptive variant trades a fixed FPR for
