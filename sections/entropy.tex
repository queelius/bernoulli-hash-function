...
\end{theorem}
\begin{proof}
For $\RV{L} = n$, every bit string of length less than $n$ must fail
...
\begin{equation}
    \Prob{\RV{L} = n} = q^{2^n - 1} \cdot (1 - q^{2^n})\,.
\end{equation}
...
\end{equation}
Alternatively, $\RV{L} = \lfloor \log_2 \RV{Q} \rfloor$ where
$\RV{Q} \sim \geodist(p)$.
\end{proof}
...
Given a BHF with bit length $\BL(\ASet{S})$ and known $\fprate$, the
...
This is achieved by skipping all salts shorter than $\ell$ bits and searching
only among salts of length exactly $\ell$.
